...
function setOrbit(s, t) {
    s.point.x = Math.sin(t) * s.orbit.x;
    s.point.z = Math.cos(t) * s.orbit.y;
}

for (var t = 0; t < 30; t++) {
    scene.objects.forEach(function(o) {
        setOrbit(o, (t / 30) * Math.PI * 2);
    });
    render(scene);
    fs.writeFileSync('frames/' + leftpad(t, 4) + '.png', c.toBuffer());
}
